...
// SetCurrencyCode .
func (provider *ProviderDefault) SetCurrencyCode(code string) {
	provider.player.CurrencyCode = code
}
...
func (provider *ProviderDefault) Auth(request *http.Request) (*pb.PlatformPlayer, error) {
	...
	provider.player.CurrencyCode = json.CurrencyCode
	provider.player.Platform = provider.ProviderName()
	provider.player.Credit, err = currency.DefaultConvertor().CurrencyToCredit(provider.player.Balance, provider.player.CurrencyCode)
...
func (provider *ProviderDefault) RequestPlayerBalance(transaction *pb.Transaction) (*pb.PlatformPlayer, error) {

	response, err := provider.request(defaultAPIPath.PlayerBalance, transaction)
	...
	err = provider.setPlayerBalanceFromResponse(response)
	...
	return provider.player, nil
}

// LockPlayerTransaction .
func (provider *ProviderDefault) LockPlayerTransaction(transaction *pb.Transaction) (*pb.PlatformPlayer, error) {

	currencyAmount, err := currency.DefaultConvertor().CreditToCurrency(transaction.Credit, provider.player.CurrencyCode)
	...
	transaction.Type = pb.TransactionType_TT_LOCK
	transaction.Amount = currencyAmount
	transaction.CurrencyCode = provider.player.CurrencyCode
	transaction.Timestamp = time.Now().Unix()

	json, err := util.ConvertProtoMessageToJSONString(transaction)
	...
	response, err := provider.request(defaultAPIPath.PlayerTransactionLock, json)

	transactionCancelOnError := &pb.Transaction{
		TransactionId:     uuid.New().String(),
		TransactionIdLock: transaction.TransactionId,
	}

	if err != nil {
		glog.Error(err)
		provider.CancelPlayerTransaction(transactionCancelOnError)
		return provider.player, err
	}
	...
	if err = LogTransaction(transaction, provider.player); err != nil {
		glog.Error(err)
	}
...
// UnlockPlayerTransaction .
func (provider *ProviderDefault) UnlockPlayerTransaction(transaction *pb.Transaction) (*pb.PlatformPlayer, error) {

	transaction.Type = pb.TransactionType_TT_UNLOCK
	transaction.CurrencyCode = provider.player.CurrencyCode
	transaction.Timestamp = time.Now().Unix()

	response, err := provider.request(defaultAPIPath.PlayerTransactionUnlock, transaction)
...
func (provider *ProviderDefault) CancelPlayerTransaction(transaction *pb.Transaction) (*pb.PlatformPlayer, error) {

	transaction.Type = pb.TransactionType_TT_CANCEL
	transaction.CurrencyCode = provider.player.CurrencyCode
	transaction.Timestamp = time.Now().Unix()

	response, err := provider.request(defaultAPIPath.PlayerTransactionCancel, transaction)
...
// PlacePlayerBet .
func (provider *ProviderDefault) PlacePlayerBet(transaction *pb.Transaction) (*pb.PlatformPlayer, error) {
	...
	transaction.Type = pb.TransactionType_TT_BET
	transaction.Amount = currencyAmount
	transaction.CurrencyCode = provider.player.CurrencyCode
	transaction.Timestamp = time.Now().Unix()
	...
	response, err := provider.request(defaultAPIPath.PlayerBetPlace, json)

	transactionCancelOnError := &pb.Transaction{
		TransactionId:        uuid.New().String(),
		TransactionIdBet:     transaction.TransactionId,
		TransactionIdBetList: []string{transaction.TransactionId},
	}

	if err != nil {
		glog.Error(err)
		provider.CancelPlayerBet(transactionCancelOnError)
		return provider.player, err
	}
...
// SettlePlayerBet .
func (provider *ProviderDefault) SettlePlayerBet(transaction *pb.Transaction) (*pb.PlatformPlayer, error) {
	...
	transaction.Type = pb.TransactionType_TT_SETTLE
	transaction.Amount = currencyAmount
	transaction.CurrencyCode = provider.player.CurrencyCode
	transaction.Timestamp = time.Now().Unix()
	...
	response, err := provider.request(defaultAPIPath.PlayerBetSettle, json)
...
// CancelPlayerBet .
func (provider *ProviderDefault) CancelPlayerBet(transaction *pb.Transaction) (*pb.PlatformPlayer, error) {

	transaction.Type = pb.TransactionType_TT_CANCEL_BET
	transaction.CurrencyCode = provider.player.CurrencyCode
	transaction.Timestamp = time.Now().Unix()

	response, err := provider.request(defaultAPIPath.PlayerBetPlace, transaction)
...
// IsPlayerLock .
func (provider *ProviderDefault) IsPlayerLock() (bool, error) {

	response, err := provider.request(defaultAPIPath.PlayerTransactionStatus, nil)
	...
	if json.Status == int32(pb.PlayerTransactionStatus_LOCK) {
		return true, nil
	}
...
func (provider *ProviderDefault) setPlayerBalanceFromResponse(response []byte) error {
	...
	if json.Error.Code > 0 {
		glog.Errorln(string(response))
		return errors.New(json.Error.Message)
	}

	credit, err := currency.DefaultConvertor().CurrencyToCredit(json.Balance, provider.player.CurrencyCode)
	...
	lockCredit, err := currency.DefaultConvertor().CurrencyToCredit(json.LockBalance, provider.player.CurrencyCode)
	...
	provider.player.Balance = json.Balance
	provider.player.LockBalance = json.LockBalance
	provider.player.Credit = credit
	provider.player.LockCredit = lockCredit
	if len(json.Icon) > 0 {
		provider.player.Icon = json.Icon
	}
