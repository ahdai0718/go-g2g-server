...
func (manager *ServerWebsocketConnectionManager) observe() {
	for {
		...
		case <-manager.intervalSyncGatewayConnection.C:
			manager.connectToGatewayWithStaticData()
...
func (manager *ServerWebsocketConnectionManager) connectToGatewayWithStaticData() {

	serverInfoMap := data.DefaultManager().GetServerInfoMap()

	serverInfoGatewayList, isExist := serverInfoMap[pb.ServerType_ST_GATEWAY]

	if isExist {
		for _, serverInfoGateway := range serverInfoGatewayList {
			manager.connectToGeteway(serverInfoGateway)
		}
	}
}
